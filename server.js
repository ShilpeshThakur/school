...
app.post('/Student_Login', function(req, res){
    ...
    var connection = getMySQLConnection();
    connection.connect();
    var qur= "select id from student where username = '"+ req.body.username+"' and password = '"+req.body.password+"'";
   // console.log(qur);
...
app.get('/Student_Profile/:id', function(req, res){
    ...
    res.status(403).json({"status_code": 500,"status_message": "unAuthorise User" });
}
    ...
    connection.query("select id from teacher where username = '"+ req.body.username+"' and password = '"+req.body.password+"'", function(err, rows, fields) {
        if (err) {
            ...
            if(rows.length>0){
                ...
                res.redirect('Student_Record')
            }
            else{
                res.redirect('/Teacher_Login');
               // res.status(401).json({"status_code": 401,"status_message": "unauthenticated user"});
            }
        }
    });
    ...
    connection.query("select id from admin where username = '"+ req.body.username+"' and password = '"+req.body.password+"'", function(err, rows, fields) {
        if (err) {
            ...
            if(rows.length>0){
                ...
                res.redirect('/Teacher_Record')
            }
            else{
                res.redirect('/Admin_Login');
               // res.status(201).json({"status_code": 401,"status_message": "unauthenticated user"});
            }
        }
    });
...
app.use(function(req,res, next) {
 ...
 if(req.session.teacher_id) {  
    ...
    //  console.log("inside 5"+req.session.teacher_id)
...
app.get('/Student_Record',function(req,res){
...
app.get('/Teacher_Profile', function(req, res){
    ...
    connection.query("select * from teacher where id = "+req.session.teacher_id, function(err, rows, fields) {
        if (err) {
            res.status(500).json({"status_code": 500,"status_message": "internal server error", 'error': err});
        }
        else{
...
app.get('/Teacher_Record',function(req,res){
...
app.get('/Admin_Add_Student', function(req, res){
...
app.get('/Admin_View_Student/:id', function(req, res){
